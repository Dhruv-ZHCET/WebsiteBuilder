...
export default function Builder() {
  ...
  const handleIndustrySelect = (industry: IndustryTemplate) => {
    setWebsiteData(prev => ({
      ...
      content: {
        ...
        footerText: `© 2024 ${prev.company.name || 'Your Company'}. All rights reserved.`
      }
    }));
  };
  ...
  const handleCompanyDetailsChange = (company: typeof websiteData.company) => {
    setWebsiteData(prev => ({
      ...
      content: {
        ...
        footerText: `© 2024 ${company.name || 'Your Company'}. All rights reserved.`
      }
    }));
  };
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 1:
        return websiteData.company.name && websiteData.company.email && websiteData.company.phone;
      case 2:
  ...
  const handlePreview = () => {
    alert('Preview functionality would show a mockup of the website with current data.');
  };
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 1:
        return <CompanyDetails companyDetails={websiteData.company} onChange={handleCompanyDetailsChange} />;
      case 2:
        return <ColorTheme selectedTheme={websiteData.colorTheme.id} onSelect={handleColorThemeSelect} />;
      case 3:
        return <ProductManagement products={websiteData.products} onChange={handleProductsChange} />;
      case 4:
        return <ContentCustomization content={websiteData.content} onChange={handleContentChange} />;
      case 5:
        return (
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-gray-600 capitalize">{websiteData.industry}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">Color Theme</h4>
                    <p className="text-gray-600">{websiteData.colorTheme.name}</p>
                  </div>
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">Products</h4>
                    <p className="text-gray-600">{websiteData.products.length} products</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="space-y-8">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                  ...
                  <div className="border border-gray-200 rounded-lg overflow-hidden">
                    <iframe src={generatedWebsite.previewUrl} className="w-full h-96" title="Website Preview" />
                  </div>
                </div>
              </div>
            )}
